...
def sample_criteria():
    return [
        ...
        Criteria(
            id="D301",
            category="Security",
            criteria="Vulnerability Scanning",
            weight=1.0,
...
@pytest.mark.parametrize(
    "url,expected_provider,expected_owner,expected_repo",
    [
        # GitHub HTTPS
        (
        ...
        # Bitbucket SSH
        (
            "git@bitbucket.org:teamname/reponame.git",
            "bitbucket",
            "teamname",
            "reponame",
        ),
...
def test_detect_remote_url_no_git_returns_none(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    with patch(
        "cli.repo_fetcher.subprocess.run",
...
def test_build_assessment_prompt_contains_criteria_ids(
    sample_criteria, sample_repo_context
):
...
def test_build_assessment_prompt_contains_repo_info(
    sample_criteria, sample_repo_context
):
    prompt = build_assessment_prompt(sample_criteria, sample_repo_context)
...
def test_build_assessment_prompt_contains_file_list(
    sample_criteria, sample_repo_context
):
    prompt = build_assessment_prompt(sample_criteria, sample_repo_context)
...
def test_build_assessment_prompt_contains_ci_content(
    sample_criteria, sample_repo_context
):
    prompt = build_assessment_prompt(sample_criteria, sample_repo_context)
...
def test_parse_ai_response_basic(sample_criteria):
    raw = json.dumps(
        {"D101": True, "D201": False, "D301": True, "suggestions": ["Add tests"]}
    )
...
def test_call_ai_openai_invokes_litellm(sample_criteria, sample_repo_context):
    ...
    with patch(
        "src.cli.ai_client.litellm.completion",
...
def test_assess_auto_requires_ai_flag():
    ...
    with patch(
        "src.cli.main.detect_remote_url", return_value="https://github.com/a/b.git"
    ):
...
def test_assess_auto_invalid_ai_provider():
    with patch(
        ...
        result = runner.invoke(
            app,
            ["assess", "--auto", "--ai", "unknown-provider", "--ai-key", "fake-key"],
        )
    assert result.exit_code != 0
...
def test_assess_auto_no_remote_url_no_provider(monkeypatch):
    ...
    with patch("src.cli.main.detect_remote_url", return_value=None):
        result = runner.invoke(
            app, ["assess", "--auto", "--ai", "ollama", "--provider", ""]
        )
    assert result.exit_code != 0
